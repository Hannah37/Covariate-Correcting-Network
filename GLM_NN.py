...
def train(dataset, args):
    ...
    log_dir = os.path.join(args.save_dir, 'tb', 'GLMNN_' + args.df + '_' + time.strftime("%Y%m%d-%H%M%S"))
    logger = SummaryWriter(log_dir=log_dir)
    dataloader = utils_data.DataLoader(dataset=dataset, batch_size=args.batch, num_workers=8, shuffle=False)
    ...
    for i in range(num_roi):
        ...
        nn.init.zeros_(Reduced_Nets[i].fc.weight)
        nn.init.zeros_(Full_Nets[i].fc.weight)
    ...
    for epoch in range (args.epoch):
        sseR_all, sseF_all = [0]*num_roi, [0]*num_roi # 148 ROIs, mean of all subjects for each ROI
        for iter_idx, data in zip(pbar, dataloader):
            ...
            print_iter_str = 'Epoch{}/{}'.format(epoch, args.epoch) \
                ...
                + ' \tRsse=%.2f' % sseR.item() \
                + ' \tFsse=%.2f' % sseF.item() \
                + ' \tlr=%.2e' % R_opts[net_idx].param_groups[0]['lr'] 
